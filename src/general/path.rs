...
impl AbPath {
    ...
    pub fn as_canonical(&self) -> Result<AbPath, Error> {
    ...
    pub fn as_directory(&self) -> AbPath {
        if self.path.is_file() {
            if let Some(parent_path) = self.path.parent() {
                let path: PathBuf = parent_path.to_path_buf();
                let is_canonical: bool = self.is_canonical;
                return AbPath { path, is_canonical };
            }
        }
        self.clone()
    }
    ...
    pub fn get_canonical_string(&self) -> Option<String> {
        let canonical_path: Result<AbPath, Error> = self.as_canonical();
        if let Ok(canonical_path) = canonical_path {
...
impl From<&Path> for AbPath {
    fn from(path: &Path) -> Self {
        AbPath::from_path(path)
    }
}

impl From<PathBuf> for AbPath {
    fn from(path_buf: PathBuf) -> Self {
        AbPath::from_path_buf(&path_buf)
    }
}
