...
impl ThreadedWalker {
    ...
    fn wait_for_threads(&self, thread_handles: &mut Vec<thread::JoinHandle<()>>) {
        loop {
            self.update_thread_handles(thread_handles);
            let active_threads: usize = thread_handles.len();
    ...
    fn spawn_walker<'a>(&'a self, root: PathBuf) {
        ...
        while let Some(current_dir) = queue.pop_front() {
            ...
            if active_threads >= self.max_threads {
                self.wait_for_threads(&mut thread_handles);
            }

            let handle: thread::JoinHandle<()> = self.add_thread(current_dir, &sender);
